...
class Application(tk.Tk):
    def __init__(self):
        ...
        # Create a timer to update sensor readings every 5 seconds
        self.sensroReader = Worker(5, self.update_sensor_readings)
        self.sensroReader.start()
    ...
    def setup_frame1_widgets(self):
        lblFrm1Title = tk.Label(self.ustFrame, text="SENSOR READS", font=("Arial", 16), bd=1, relief="raised")
        lblFrm1Title.grid(row=0, column=0, columnspan=6, padx=5, pady=5, sticky="nsew")

        labels = ["CO2", "Ph", "Su S", "Ort S", "Nem", "EC"]
        for i, text in enumerate(labels):
            tk.Label(self.ustFrame, text=text, font=("Arial", 16), bd=1, relief="solid").grid(row=1, column=i, padx=5, pady=5, sticky="nswe")
    ...
    def setup_frame2_widgets(self):
        ...
        labels = ["CO2", "Ph", "Su S", "Ort S", "Nem", "EC"]
        for i, text in enumerate(labels):
            tk.Label(self.ortaFrame, text=text, font=("Arial", 16), bd=1, relief="solid").grid(row=1, column=i, padx=5, pady=5, sticky="nswe")

        self.entries = []
        for i in range(6):
            entry = tk.Entry(self.ortaFrame, width=8, font=("Arial", 16),justify="center", relief="sunken")
    ...
    def setup_frame3_widgets(self):
        ...
        for i, text in enumerate(labels):
            tk.Label(self.altFrame, text=text, font=("Arial", 16), bd=1, relief="solid").grid(row=1, column=i, padx=5, pady=5, sticky="nswe")
